...
function MultilineTrigger:Inject()
  ...
  for i = 1, #self.lines, 1 do
    ...
    if (i == 1) then
      ...
      AddTriggerEx(tgroupname, self.lines[i].text, 'EnableGroup("'..groupname..'", 1)', flags, NOCHANGE, 0, '', '', sendto.script, self.sequence)
      SetTriggerOption(tgroupname, 'group', self.group)
    elseif (i == #self.lines) then
      AddTriggerEx(tgroupname, self.lines[i].text, 'EnableGroup("'..groupname..'", 0)\r\n'..self.script, flags, NOCHANGE, 0, '', '', sendto. script, self.sequence)
      SetTriggerOption(tgroupname, 'group', groupname)
    else
      AddTriggerEx(tgroupname, self.lines[i].text, '', flags, NOCHANGE, 0, '', '', sendto.script, self.sequence)
      SetTriggerOption(tgroupname, 'group', groupname)
    end
  end
end
